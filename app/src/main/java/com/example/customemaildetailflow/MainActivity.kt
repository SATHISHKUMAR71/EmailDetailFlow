...
//            outState.putString("date$i",email.date)
//            outState.putString("title$i",email.title)
//            outState.putString("subtitle$i",email.subtitle)
//            outState.putString("content$i",email.content)
//            outState.putBoolean("isStarred$i",email.isStarred)
//            outState.putBoolean("isViewed$i",email.isViewed)
//            println("Email List: Outstate: $i i value")
//            i+=1
//        }
//    }
    ...
    override fun displayDetails(
        ...
        if(isDualPane){
            fragmentEmailDetail.putData(title,subtitle,date,content,isStarred,isViewed,transitionName)
        }
        else{
            fragmentEmailDetail.arguments?.putString("transitionName",transitionName)
            supportFragmentManager.beginTransaction()
                .addSharedElement(email,transitionName)
                .addToBackStack("Detail Fragment")
                .replace(R.id.fragmentEmailList,fragmentEmailDetail)
                .commit()
            supportFragmentManager.executePendingTransactions()

            appbar?.findViewById<MaterialToolbar?>(R.id.toolbar)?.apply {
                this.title = title
                navigationIcon = ContextCompat.getDrawable(baseContext,R.drawable.baseline_arrow_back_24)
            }
            println("$title,$subtitle,$date,$content,$isStarred,$isViewed,$transitionName")
            fragmentEmailDetail.putData(title,subtitle,date,content,isStarred,isViewed,transitionName)
        }
    }

//    private fun initCheck(savedInstanceState: Bundle?) {
//        if(savedInstanceState != null){
//            println("In Saved onCreate Instance")
//            savedInstanceState.getInt("size")
//            for(i in 0..<savedInstanceState.getInt("size")){
//                val date = savedInstanceState.getString("date$i")?:"null"
//                val title = savedInstanceState.getString("title$i")?:"null"
//                val subtitle = savedInstanceState.getString("subtitle$i")?:"null"
//                val content = savedInstanceState.getString("content$i")?:"null"
//                val isStarred = savedInstanceState.getBoolean("isStarred$i")
//                val isViewed = savedInstanceState.getBoolean("isViewed$i")
//                val email = Email(title,subtitle,date,content,isStarred,isViewed)
//                emailList.add(email)
//                println("Email List Restore i value: $i")
//                println("Email List ${email}")
//            }
//        }
//        else{
...
////            emailList.add(Email("Googlesion and Pattern Rsion and Pattern ","New Sign in Activity","07 September","Hi Satish is that you signed your google account on the device MK210P",false,false))
